        ...
        <div className='flex flex-col'>
          ...
          <div className='flex flex-col gap-y-6'>
            {state.question.answers.map((answer) => (
              <AnswerListItem key={answer.id} answerItem={answer} />
            ))}
          </div>
          <div>
            ...
            <textarea
              ...
              className={`w-full h-40 border ${
                showAnswerInputError ? 'border-errorText' : 'border-textAreaBorder'
              } mt-4 p-4 rounded-md`}
              placeholder='متن پاسخ ... '
            />
            {showAnswerInputError && (
              <span className='text-errorText text-xs my-2'>لطفا پاسخ را وارد کنید</span>
            )}
            <Button
