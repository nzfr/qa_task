    ...
    return <Card>
        ...
        <CardContent>
            ...
            <div className='container grid grid-cols-5 gap-2 mx-auto'>
                {showImages && questionItem.images.map((image, index) => {
                    return <img className='w-52 h-52' alt={`question_${questionItem.id}_image_${index}`} key={index}
                                src={image}/>
                })}
            </div>
            {!showImages ? <div className='flex flex-row justify-end items-center'>
                <Button onClick={() => navigator(`/question-details/${questionItem.id}`)} variant='Outlined'
                        type="Success" title='مشاهده جزییات'/>
            </div> : <></>}
        </CardContent>
    </Card>
}
