...
const NewQuestionModal = () => {
    ...
    const submitDisabled = !titleInput || !contentInput || mainState.newQuestionModal.state === 'loading';

    return <Modal showCloseButton={true} title='ایجاد سوال جدید' onDismiss={() => dispatch(toggleDialog(false))}>
        <ModalContent>
        ...
        <ModalFooter>
            <Button disabled={submitDisabled} buttonType='submit' form='new-question-form' className={`w-24 ${submitDisabled && 'cursor-not-allowed'}`} variant='Filled'
                    title='ایجاد سوال' type="Success"/>
            <Button disabled={mainState.newQuestionModal.state === 'loading'} className='w-24' title='انصراف' variant='Filled' onClick={() => dispatch(toggleDialog(false))}
                    type="Error"/>
        </ModalFooter>
    </Modal>
}
